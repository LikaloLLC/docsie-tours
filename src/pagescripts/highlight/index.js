...
function getPanelEl() {
  if (!_panel) {
    ...
    _panel.innerHTML = PanelStyle + "<div id='__managerExtentionPanel__body'></div>";
...
function updatePanel(path) {
  ...
  if (!!path) {
    const view = "<div class='__managerExtentionPanel__path'>" + frags.map(function (frag, index) {
      return "<a class='__managerExtentionPanel__path_item' data-index='" + index + "'><span class='breadcrumb__inner'><span class='breadcrumb__title'>" + frag + "</span></span></a>"
    }).join("") + "</div>";
...
function getDomPath(el) {
  ...
  while (loop) {
    ...
    if (el.hasAttribute('id') && el.id != '') {
      ...
      stack.unshift(el.nodeName.toLowerCase() + getPseudo(sibIndex, el.parentNode.childNodes.length));
    } else {
